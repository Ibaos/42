...
void	initialize_window_entitites(t_window_info *win)
{
	...
	if (win->map.monster.x && win->map.monster.y)
	{
		mlx_put_image_to_window(win->mlx_ptr, win->win_ptr,
			win->map.monster.right, win->map.monster.x, win->map.monster.y);
	}
}

void	check_map_all(t_window_info *win)
{
	win->map.monster.count = 0;
	win->map.monster.x = 0;
	win->map.monster.y = 0;
...
void	mlx_start(t_window_info *win)
{
	...
	mlx_string_put(win->mlx_ptr, win->win_ptr, 10, 20, 0xFFFFFF, "Move: 0");
